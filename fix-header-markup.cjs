...
files.forEach(file => {
  ...
  // Make sure we have the correct CSS
  content = content.replace(/\/\* ── Site Header \(Exactly matches React\) ── \*\/[\s\S]*?\/\* ── Page Hero ── \*\//, cssReplacement);
  
  // Make sure the HTML is actually inserted directly below the body tag
  const htmlNavReplacement = `<body>
<nav class="react-header-wrapper">
  ...
  // Clear out the previous nav attempts entirely
  content = content.replace(/<nav>[\s\S]*?<\/nav>/, '');
  content = content.replace(/<nav class="react-header-wrapper">[\s\S]*?<\/nav>/, '');
  content = content.replace(/<nav class="site-header">[\s\S]*?<\/nav>/, '');
  
  // Insert exactly after <body> to make sure it's the first element and has no layout interruptions
  content = content.replace(/<body>\s*/, htmlNavReplacement + '\n');
